...
async function buildPipeline() {
    console.log(`[FuelRatsV3] Starting Compilation`);
    ...
    if (!process.argv.includes('WATCHING')) {
        ...
        console.log(`[FuelRatsV3] Compiling Typescript`);
    ...
    // Handle Source Copy
    console.log(`[FuelRatsV3] Copy Compiled Content`);
    ...
    console.log(`[FuelRatsV3] Copying Addon Resources`);
    fs.copySync(path.join(MainPath, 'addon-resources'), path.join(MainPath, 'resources'), { recursive: true });
    console.log(`[FuelRatsV3] Copied ${copiedFiles} Extra Files for FuelRatsV3`);

    if (compilationPromise) {
        await compilationPromise;
    }

    const CompiledFiles = new glob.GlobSync('./fuelrats-cache/**/*.!(ts)').found;
    for (let i = 0; i < CompiledFiles.length; i++) {
        const oldPath = CompiledFiles[i];
        const newPath = CompiledFiles[i].replace('fuelrats-cache', 'resources');
        const dirName = path.dirname(newPath).normalize();
    ...
    console.log(`[FuelRatsV3] Build Time: ${Date.now() - StartTime}ms`);
    console.log(`[FuelRatsV3] Attempting to Boot Server...`);
}
